    ...
    <div className="order-ui mx-auto max-w-md space-y-3 pb-20">
      ...
      <div className="grid grid-cols-3 gap-2 px-1">
        <div className="rounded-lg border border-blue-100 bg-blue-50/50 py-2 px-3 transition-all">
          <div className="text-[10px] uppercase tracking-wider text-blue-600 font-bold">ทั้งหมด</div>
          <div className="text-lg font-black text-blue-800">{orders.length}</div>
        </div>
        <div className="rounded-lg border border-amber-100 bg-amber-50/50 py-2 px-3 transition-all">
          <div className="text-[10px] uppercase tracking-wider text-amber-600 font-bold">ค้างจ่าย</div>
          <div className="text-lg font-black text-amber-800">{activeOrders.length}</div>
        </div>
        <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 py-2 px-3 transition-all">
          <div className="text-[10px] uppercase tracking-wider text-emerald-600 font-bold">เสร็จสิ้น</div>
          <div className="text-lg font-black text-emerald-800">{completedOrders.length}</div>
        </div>
      </div>
              ...
              >
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="text-[13px] font-semibold text-slate-600 truncate space-y-0.5">
                    <div><span className="opacity-50 text-[10px] uppercase font-black mr-1">ผู้สั่ง:</span> {order.requesterName || "-"}</div>
                       ...
                       {(() => {
                         ...
                         if (order.status === "completed" || order.status === "sorting") {
                           if (missing === 0 && bought > 0) {
                             return <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded uppercase">ครบ</span>;
                           }
                           return (
                         ...
                         return <span className="text-[10px] font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">{progress}/{itemCount} Items</span>;
                       })()}
                    </div>
        ...
        {selectedOrder && (
          <div className="space-y-4">
              ...
              <>
                <div className="grid grid-cols-3 gap-1.5 text-[10px]">
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                      <span className="text-slate-500 block mb-1 font-black uppercase tracking-wider">สถานที่</span>
                      <span className="font-bold text-slate-900 truncate block text-[11px]">{selectedOrder.location || "N/A"}</span>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                      <span className="text-slate-500 block mb-1 font-black uppercase tracking-wider">สั่งเมื่อ</span>
                      <span className="font-bold text-slate-900 truncate block text-[11px]">{formatDateTime(selectedOrder.createdAt)}</span>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                      <span className="text-slate-500 block mb-1 font-black uppercase tracking-wider">ผู้สั่ง</span>
            ...
            <div className="space-y-1.5">
              <span className="text-[11px] font-black text-slate-500 px-1 uppercase tracking-widest">หมายเหตุเพิ่มเติม</span>
              <textarea
            ...
            <div className="pt-2 flex gap-3">
              <button
                ...
                className="flex-1 h-12 px-4 rounded-xl border-2 border-slate-200 text-sm font-black text-slate-600 hover:bg-slate-50 transition-all active:scale-[0.98]"
              >
                ยกเลิก
              </button>
              <Button
